...
(function () {
	...
	ns.NodeDecorator = {
		//field
		_searcher : ns.TreeSearcher,
		_styleAppender : ns.StyleAppender,
		_startOffset : null,
		_endOffset : null,
		...
		decorateAllNodeInLines : function (lines, style) {
			for(var i in lines) {
				...
				this._styleAppender.appendStyleToElement(span,style);
			};
		...
		decorateLineBy : function(range, style) {
			...
			for(var i in lineNodes) {
				this._styleAppender.appendStyleToElement(lineNodes[i],style);
			}
		...
		_linesInRange : function (range) {
		    ...
		    return this._searcher.searchTree(startNode, endNode, filteringLine);
		...
		_nodesInRange : function (range) {
			...
			return this._searcher.searchTree(startNode, endNode);
		},
		_decorateSpan : function (textNodes,style) {
			...
			for(var i =startIndex; i < size; ++i) {
				...
				this._styleAppender.appendStyleToElement(spanNode,style);				//TODO: 성능 : 리플로우 리페인팅이 일어남..
